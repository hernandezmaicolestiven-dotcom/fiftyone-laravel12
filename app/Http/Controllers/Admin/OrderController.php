...
class OrderController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        ...
        $filters  = [];
        if ($request->filled('search'))    $filters[] = 'Cliente: ' . $request->search;
        if ($request->filled('status'))    $filters[] = 'Estado: ' . ($orders->first()?->status_label ?? $request->status);
        if ($request->filled('date_from')) $filters[] = 'Desde: ' . \Carbon\Carbon::parse($request->date_from)->format('d/m/Y');
        if ($request->filled('date_to'))   $filters[] = 'Hasta: ' . \Carbon\Carbon::parse($request->date_to)->format('d/m/Y');
