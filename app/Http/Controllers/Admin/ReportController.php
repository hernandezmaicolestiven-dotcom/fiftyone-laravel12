...
class ReportController extends Controller
{
    ...
    public function sales(Request $request)
    {
        ...
        $chartOrders  = $days->map(fn($d) => $orders->filter(fn($o) =>
            Carbon::parse($o->created_at)->isSameDay($d))->count())->values();
        $chartRevenue = $days->map(fn($d) => (float) $orders->filter(fn($o) =>
            Carbon::parse($o->created_at)->isSameDay($d))->sum('total'))->values();
    ...
    public function inventory(Request $request)
    {
        ...
        if ($stockFilter === 'out')  $query->where('stock', 0);
        elseif ($stockFilter === 'low')  $query->where('stock', '>', 0)->where('stock', '<', 5);
        elseif ($stockFilter === 'ok')   $query->where('stock', '>=', 5);
    ...
    public function topProducts(Request $request)
    {
        ...
        $topProducts = $items->groupBy('product_name')
            ...
            ->when($categoryId, fn($col) => $col->filter(fn($p) =>
                $items->where('product_name', $p['name'])->first()?->product?->category_id == $categoryId
            ))
