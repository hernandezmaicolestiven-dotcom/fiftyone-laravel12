...
<div class="bg-white rounded-xl shadow-sm">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-gray-100">
        <div>
            ...
            <p class="text-sm text-gray-500">{{ $categories->total() }} categorias en total</p>
        </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-gray-500 text-xs max-w-xs truncate">{{ $category->description ?? '-' }}</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form id="del-cat-{{ $category->id }}" method="POST"
                                ...
                                @csrf @method('DELETE')
                                <button type="button"
                                        @click="$dispatch('confirm-delete', { form: document.getElementById('del-cat-{{ $category->id }}'), msg: 'La categoria {{ addslashes($category->name) }} se movera a la papelera. Los productos quedaran sin categoria.' })"
