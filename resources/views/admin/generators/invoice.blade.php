...
<div class="body">
    <div class="grid2">
        ...
        <div class="box">
            ...
            <p style="margin-bottom:6px"><strong>Pedido #{{ $order->id }}</strong></p>
            <span class="status" style="background:{{ match($order->status){ 'delivered'=>'#d1fae5','shipped'=>'#e0e7ff','confirmed'=>'#dbeafe','cancelled'=>'#fee2e2',default=>'#fef3c7'} }};color:{{ match($order->status){'delivered'=>'#065f46','shipped'=>'#3730a3','confirmed'=>'#1e40af','cancelled'=>'#991b1b',default=>'#92400e'} }}">
                {{ $order->status_label }}
            </span>
        </div>
    </div>
    ...
    <div class="total-box">
        <div>
            ...
            @if($order->notes)<div style="font-size:11px;color:#94a3b8;margin-top:4px">Nota: {{ $order->notes }}</div>@endif
        </div>
