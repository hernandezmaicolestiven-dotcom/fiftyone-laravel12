...
<div class="flex gap-1 bg-white rounded-2xl shadow-sm p-1.5 mb-6 overflow-x-auto">
    ...
    ] as [$key, $icon, $label, $activeText])
    <button @click="tab='{{ $key }}'"
            :class="tab==='{{ $key }}'
                ? '{{ $key === 'danger' ? 'bg-red-500 text-white shadow-sm' : 'text-white shadow-sm' }}'
            ...
            <form method="POST" action="{{ route('admin.settings.profile') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        @error('name')<p class="text-red-500 text-xs mt-1.5 flex items-center gap-1"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" required
                                   class="w-full pl-11 pr-4 py-3.5 border-2 rounded-xl text-sm font-medium focus:outline-none transition
                                          {{ $errors->has('email') ? 'border-red-300 bg-red-50' : 'border-gray-100 bg-gray-50 focus:border-indigo-400 focus:bg-white' }}">
                        </div>
        ...
        <div class="p-6">
            ...
            <form method="POST" action="{{ route('admin.settings.password') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <input type="password" name="current_password" id="cur_pass"
                               class="w-full pl-11 pr-12 py-3.5 border-2 rounded-xl text-sm font-medium focus:outline-none transition
                                      {{ $errors->has('current_password') ? 'border-red-300 bg-red-50' : 'border-gray-100 bg-gray-50 focus:border-purple-400 focus:bg-white' }}">
                    ...
                    @error('current_password')<p class="text-red-500 text-xs mt-1.5 flex items-center gap-1"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative group">
                            ...
                            <input type="password" name="password" id="new_pass"
                                   class="w-full pl-11 pr-12 py-3.5 border-2 rounded-xl text-sm font-medium focus:outline-none transition
                                          {{ $errors->has('password') ? 'border-red-300 bg-red-50' : 'border-gray-100 bg-gray-50 focus:border-purple-400 focus:bg-white' }}">
...
<div x-show="tab==='store'" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="p-6 space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <span class="absolute inset-y-0 right-4 flex items-center">
                        <code class="text-xs bg-gray-200 text-gray-500 px-2 py-0.5 rounded-lg">{{ $key }}</code>
                    </span>
                </div>
            </div>
            @endforeach
            <div class="flex items-center gap-3 flex-wrap">
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold
                    {{ config('app.env') === 'production' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                    <span class="w-2 h-2 rounded-full {{ config('app.env') === 'production' ? 'bg-emerald-500' : 'bg-amber-500' }} animate-pulse"></span>
                    {{ ucfirst(config('app.env')) }}
                </span>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold
                    {{ config('app.debug') ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-600' }}">
    ...
    <div class="grid grid-cols-3 gap-4 mt-5">
        ...
        <a href="{{ $url }}"
           class="flex flex-col items-center gap-3 p-5 rounded-2xl border-2 transition group {{ $cls }}">
            <i class="fa-solid {{ $icon }} text-2xl opacity-60 group-hover:opacity-100 transition"></i>
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden border-2 border-red-100">
        ...
        <div class="p-6">
            ...
            <div class="rounded-2xl border-2 border-red-100 bg-red-50 p-5">
                ...
                <form method="POST" action="{{ route('admin.settings.delete') }}"
                    ...
                    @error('confirm_delete')<p class="text-red-500 text-xs mt-2">{{ $message }}</p>@enderror
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="p-5">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between py-2.5 border-b border-gray-50">
                    ...
                    <span class="text-xs font-bold text-gray-700 bg-gray-100 px-2.5 py-1 rounded-lg">#{{ $user->id }}</span>
                </div>
                <div class="flex items-center justify-between py-2.5 border-b border-gray-50">
                    <span class="text-xs text-gray-400 flex items-center gap-2">
                ...
                <div class="flex items-center justify-between py-2.5 border-b border-gray-50">
                    ...
                    <span class="text-xs font-bold text-gray-700">{{ $user->created_at->format('d/m/Y') }}</span>
                </div>
                <div class="flex items-center justify-between py-2.5">
                    <span class="text-xs text-gray-400 flex items-center gap-2">
                        <i class="fa-solid fa-pen text-sky-400 w-4 text-center"></i> Editado
                    </span>
                    <span class="text-xs font-bold text-gray-700">{{ $user->updated_at->diffForHumans() }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-5" x-show="tab==='security'">
        ...
        <ul class="space-y-3">
            ...
            <li class="flex items-center gap-2.5 text-xs text-gray-500">
                <i class="fa-solid {{ $icon }} {{ $color }} flex-shrink-0"></i> {{ $tip }}
            </li>
            @endforeach
        </ul>
    </div>
        ...
        <div class="space-y-2">
            ...
            <a href="{{ $url }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-500 hover:bg-gray-50 hover:text-indigo-600 transition group">
                <i class="fa-solid {{ $icon }} w-4 text-center text-gray-300 group-hover:text-indigo-500 transition"></i>
                {{ $lbl }}
